...
def Guess(participants):
    my_participant_dict = {}
    assert type(participants) == list, "Participants must be a list"
#     assert "Larry" in participants, None
    for participant in participants:
        assert type(participant) == str, "Name of participant must be a list of string"
        my_participant_dict[participant] = random.randint(1, 9)
    try:
        if my_participant_dict['Larry'] == 9:
            return True
